module github.com/vkuznet/auth-proxy-server

go 1.19

require (
	github.com/coreos/go-oidc v2.2.1+incompatible
	github.com/dmwm/cmsauth v0.0.0-20220120183156-5495692d4ca7
	github.com/go-ole/go-ole v1.2.6 // indirect
	github.com/golang-jwt/jwt v3.2.2+incompatible
	github.com/golang/protobuf v1.5.2 // indirect
	github.com/google/uuid v1.3.0
	github.com/lestrrat-go/file-rotatelogs v2.4.0+incompatible
	github.com/pquerna/cachecontrol v0.1.0 // indirect
	github.com/shirou/gopsutil v3.21.11+incompatible
	github.com/stretchr/testify v1.7.0
	github.com/thomasdarimont/go-kc-example v0.0.0-20170529223628-e3951d8faa4c
	github.com/tklauser/go-sysconf v0.3.11 // indirect
	github.com/vkuznet/auth-proxy-server/auth v0.0.0-20230218132251-afaaef6261a0
	github.com/vkuznet/auth-proxy-server/logging v0.0.0-20230218132251-afaaef6261a0
	github.com/vkuznet/x509proxy v0.0.0-20210801171832-e47b94db99b6 // indirect
	golang.org/x/crypto v0.6.0
	golang.org/x/net v0.7.0 // indirect
	golang.org/x/oauth2 v0.5.0
	golang.org/x/sys v0.5.0 // indirect
	google.golang.org/appengine v1.6.7 // indirect
	google.golang.org/protobuf v1.28.1 // indirect
	gopkg.in/square/go-jose.v2 v2.6.0 // indirect
)

require github.com/vkuznet/auth-proxy-server/cric v0.0.0-20230218132251-afaaef6261a0

require (
	github.com/davecgh/go-spew v1.1.0 // indirect
	github.com/lestrrat-go/strftime v1.0.6 // indirect
	github.com/pascaldekloe/jwt v1.12.0 // indirect
	github.com/pkg/errors v0.9.1 // indirect
	github.com/pmezard/go-difflib v1.0.0 // indirect
	github.com/tklauser/numcpus v0.6.0 // indirect
	github.com/yusufpapurcu/wmi v1.2.2 // indirect
	golang.org/x/text v0.7.0 // indirect
	gopkg.in/yaml.v3 v3.0.0-20200313102051-9f266ea9e77c // indirect
)
